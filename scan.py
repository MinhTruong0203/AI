...
class DocScanner(object):
    ...
    def interactive_get_contour(self, screenCnt, rescaled_image):
        ...
        fig, ax = plt.subplots()
        ax.add_patch(poly)
        ax.set_title(('Hay chon vung chua hinh và đóng hộp thoại để xử lý'
            ))
    ...
    def scan(self, image_path):
        ...
        cv2.imwrite('Temp/scan.jpg',threshed)
        #print("Proccessed " + basename)
        
        # apply the perspective transformation
        warped1 = transform.four_point_transform(orig, screenCnt * ratio)

        # convert the warped image to grayscale
        gray1 = cv2.cvtColor(warped1, cv2.COLOR_BGR2GRAY)

        # sharpen image
        sharpen1 = cv2.GaussianBlur(gray1, (0,0), 3)
        sharpen1 = cv2.addWeighted(gray1, 1.5, sharpen, -0.5, 0)

        # apply adaptive threshold to get black and white effect
        thresh = cv2.adaptiveThreshold(sharpen1, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 15)

        # save the transformed image
        cv2.imwrite('static/media/reload.jpg' , thresh)
